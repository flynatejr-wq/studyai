  ...
  return (
    <div className="space-y-4">
      ...
      <div className={`border rounded-2xl p-5 ${card}`}>
        <h3 className={`font-bold mb-3 flex items-center gap-2 ${dark ? "text-white" : "text-gray-800"}`}>ðŸ“ Summary</h3>
        <ul className="space-y-2">
          {summary.map((p, i) => (
            <li key={i} className={`flex items-start gap-2 text-sm ${text}`}>
              <span className="text-indigo-400 mt-0.5">â€¢</span><span>{p}</span>
            </li>
          ))}
        </ul>
      </div>
      ...
      {/* Key Terms */}
      <div className={`border rounded-2xl p-5 ${card}`}>
        <h3 className={`font-bold mb-3 ${dark ? "text-white" : "text-gray-800"}`}>ðŸ”‘ Key Terms</h3>
      ...
      {/* Quiz */}
      <div className={`border rounded-2xl p-5 ${card}`}>
        <h3 className={`font-bold mb-3 ${dark ? "text-white" : "text-gray-800"}`}>ðŸ§  Quiz Preview</h3>
        <div className="space-y-2">
          {questions.map((q, i) => (
            <div key={i} onClick={() => setFlipped(f => ({ ...f, [i]: !f[i] }))}
              ...
              {flipped[i] && <p className="text-indigo-400 text-sm mt-2 pt-2 border-t border-white/10">âœ… {q.answer}</p>}
