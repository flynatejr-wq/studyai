...
import { BookOpen, LayoutDashboard, BarChart2, LogOut, Zap, Menu, X, Settings } from "lucide-react";
    ...
    <>
      ...
      <header className="md:hidden fixed top-0 left-0 right-0 h-14 bg-slate-900 border-b border-white/10 flex items-center justify-between px-4 z-40">
        ...
        <Link to="/dashboard" className="flex items-center gap-2 font-bold">
          ...
          <span className="bg-gradient-to-r from-indigo-400 to-violet-400 bg-clip-text text-transparent text-base">StudyBuddi</span>
        </Link>
      ...
      {/* Mobile backdrop */}
      {open && (
        <div className="md:hidden fixed inset-0 bg-black/60 z-40 backdrop-blur-sm" onClick={close} />
      )}
      ...
      `}>
        ...
        <div className="px-4 py-4 border-t border-white/10">
          <div className="bg-white/5 rounded-2xl p-3 mb-3">
            ...
            {user?.streak > 0 && (
              <p className="text-center text-xs text-orange-400 mt-2 font-medium">ðŸ”¥ {user.streak} day streak!</p>
            )}
          </div>
          ...
          <div className="flex items-center justify-between px-1">
            ...
            <button onClick={onLogout} title="Sign out"
              className="text-gray-500 hover:text-red-400 transition-colors p-1.5 rounded-lg hover:bg-red-400/10 shrink-0 ml-2">
              <LogOut size={16} />
            </button>
          </div>
        </div>
