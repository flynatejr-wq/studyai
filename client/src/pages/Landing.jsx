...
const features = [
  { icon: Zap, title: "Instant Study Guides", desc: "Paste notes, upload photos, or drop in audio — get a full study guide in seconds.", color: "text-yellow-500" },
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-900 via-indigo-950 to-slate-900 text-white">
      ...
      <nav className="flex items-center justify-between px-8 py-5 max-w-6xl mx-auto">
        ...
        <div className="flex gap-3">
          <Link to="/login" className="px-5 py-2 rounded-xl text-gray-300 hover:text-white transition-colors font-medium">Log in</Link>
          <Link to="/signup" className="px-5 py-2 rounded-xl bg-indigo-600 hover:bg-indigo-500 transition-colors font-semibold">Get Started Free</Link>
        </div>
      </nav>
      ...
      <section className="text-center px-6 pt-20 pb-24 max-w-4xl mx-auto">
        <motion.div initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6 }}>
          <span className="inline-block bg-indigo-500/20 text-indigo-300 border border-indigo-500/30 rounded-full px-4 py-1.5 text-sm font-medium mb-6">
          ...
          <p className="text-xl text-gray-400 mb-10 max-w-2xl mx-auto">
            Upload audio, snap a photo, or paste your notes. StudyAI creates summaries, flashcards, quizzes, and an AI tutor — instantly.
          </p>
          <div className="flex flex-col sm:flex-row gap-4 justify-center">
            <Link to="/signup" className="px-8 py-4 rounded-2xl bg-gradient-to-r from-indigo-600 to-violet-600 hover:from-indigo-500 hover:to-violet-500 transition-all font-bold text-lg shadow-lg shadow-indigo-500/25">
              Start Studying Free →
            </Link>
            <Link to="/login" className="px-8 py-4 rounded-2xl border border-white/10 hover:border-white/20 hover:bg-white/5 transition-all font-semibold text-lg">
              I have an account
            </Link>
          </div>
        </motion.div>
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.4, duration: 0.5 }}
          ...
          {[["📚", "Summarize instantly"], ["🧠", "Quiz yourself"], ["🔥", "Build streaks"], ["💬", "AI tutor included"]].map(([icon, label]) => (
